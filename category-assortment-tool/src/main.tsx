...
function buildAnalysis(rows:NormalizedRow[]):AnalysisRow[]{
  ...
  base.forEach(r=>{
    ...
    if(sp===null || sc===null || up===null || uc===null){r.recommendation='DATOS INSUFICIENTES';r.reason='Hay ventas o unidades vacías/no numéricas. El motor no asume que un dato faltante sea cero.';return}
    if(sp===0 && sc>0){r.recommendation='INNOVACIÓN';r.reason='Venta anterior = 0 y venta actual > 0. Se protege para revisión comercial.';return}
    if(sp>0 && sc===0){r.recommendation='DESCODIFICADO';r.reason='Tenía venta en el período anterior y no registra venta actual.';return}
...
function App(){
 ...
 return <div className="app"><header><div><span className="eyebrow">RAMO · CATEGORY MANAGEMENT</span><h1>Assortment Tool <b>V0.2</b></h1></div><div className="privacy">● Procesamiento local · sin base de datos</div></header>
 ...
 {step===5&&<section><div className="sectionTitle"><div><span className="tag success">NORMALIZADO</span><h2>{stats.rows.toLocaleString()} registros listos para el motor</h2><p>La fuente original permanece intacta. Los datos faltantes se conservan como faltantes; nunca se convierten silenciosamente en cero.</p></div><div className="actions"><button className="secondary" onClick={()=>setStep(3)}>← Ajustar mapeo</button><button className="primary" onClick={()=>setStep(6)}>Analizar surtido →</button></div></div><Preview rows={normalized}/></section>}
...
function Analysis({rows,allRows,counts,filter,setFilter}:{rows:AnalysisRow[];allRows:AnalysisRow[];counts:Record<Recommendation,number>;filter:'TODOS'|Recommendation;setFilter:(v:'TODOS'|Recommendation)=>void}){
 ...
 return <section><div className="sectionTitle"><div><span className="tag">MOTOR DE SURTIDO</span><h2>Recomendación automática</h2><p>{allRows.length} SKU analizados · {comparable} con historia comparable. La recomendación es cuantitativa y todavía no es la decisión final.</p></div></div>
 ...
 <div className="done">✓ Sprint 2 inicial: métricas, Pareto por segmento, casos especiales y recomendación explicada funcionando. Siguiente validación: comparar estas recomendaciones contra el ejercicio manual.</div></section>
}
